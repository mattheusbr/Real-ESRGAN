...
def get_sub_video(args, num_process, process_idx):
    ...
    cmd = [
        ...
        '-to', str(part_time * (process_idx + 1)) if process_idx != num_process - 1 else '',
        '-async', '1', out_path, '-y'
    ]
...
def inference_stream(args, upsampler, face_enhancer):
    ...
    if is_video:
        ...
        decoder = (
            ffmpeg.input(args.input).output('pipe:', format='rawvideo', pix_fmt='rgb24', loglevel='warning')
            .run_async(pipe_stdin=True, pipe_stdout=True, cmd=args.ffmpeg_bin))
    else:
        from PIL import Image
    ...
    if is_video:
        ...
        encoder = (
            ffmpeg.input('pipe:', format='rawvideo', pix_fmt='rgb24', s=f'{out_width}x{out_height}', framerate=args.fps)
    ...
    while True:
        ...
        try:
            ...
            encoder.stdin.write(output.astype(np.uint8).tobytes())
